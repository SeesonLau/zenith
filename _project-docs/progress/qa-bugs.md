...
| ID | File | Line | Issue | Severity | If Unaddressed | Fix Proposal | Status |
|---|---|---|---|---|---|---|---|
| QA-051 | `src/database/sync/supabaseSync.ts` | 151 | `migrationsEnabledAtVersion: 5` but schema version is `6`. This value means "devices on schema v5 get the v6 migration; devices below v5 get their DB reset." Correct as-is — schema started at v5 so no devices exist below that. | High | No real impact — value is correct for this project's history | **Deferred — by design.** Schema started at v5; `migrationsEnabledAtVersion: 5` is correct. No action needed. | Deferred |
| QA-052 | `src/database/migrations.ts` | — | Only one migration step exists (`toVersion: 6`). No migration steps for v1–v4. Devices on schema v1–v4 would have their DB reset on upgrade. | Medium | Dev builds on very old schema versions lose local data on upgrade | **Deferred — no fix possible.** Migration history for v1–v4 does not exist in the codebase. Since this is a personal app and the schema was at v5 from the first usable build, no devices with v1–v4 exist. Document only. | Deferred |
| QA-053 | `src/database/actions/diaryActions.ts` | 18–49 | `createDiaryEntry` calls `addImageToDiaryEntry` inside a `database.write(...)` block. `addImageToDiaryEntry` is itself an independent `database.write(...)` call — nested writes are not allowed in WatermelonDB and will throw. | Critical | App crashes when creating a diary entry with images | Move image creation logic inline inside the outer `database.write`, or batch using `database.batch()` | ✅ Fixed 2026-03-28 |
